...
{
  usleep(500);
  if (PageTable[param].bVaild == false)
    return -1;
...
void fFIFO(int iPage)
{
  ...
  for (int i = 0; i < PAGE_TABLE_SIZE; i++)
    if (PageTable[i].iFrame == CLOCK_PTB)
      PageTable[i].bVaild = false; // Clear Victim from Page Table
  for (int i = 0; i < TLB_SIZE; i++)
    if (TLB[i].iFrame == CLOCK_PTB)
      TLB[i].bVaild = false; // Clear Victim from TLB

  if (PageTable[iPage].bDirty == true)
    fSwapOut(iPage); // Swap out if the victim page has been modified
...
int main(int argc, char *argv[])
{
  ...
  if (SEQ > 0)
  {
    ...
    initalizer();
    for (int i = 0; i < SEQ; i++)
      LogicMem[i] = rand() % 65536;

    for (int i = 0; i < SEQ; i++)
      fTranslate(LogicMem[i]);
